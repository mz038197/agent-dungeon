...
class GoogleOAuthService:
    ...
    def verify_state(self, state: str) -> bool:
        for candidate in self._decode_state_candidates(state):
            if self._verify_state_raw(candidate):
                return True
        return False

    def _verify_state_raw(self, state: str) -> bool:
        parts = state.split(":")
        if len(parts) != 3:
            return False
        ...
        return hmac.compare_digest(expected.lower(), sig.strip().lower())
    ...
    def exchange_code(self, code: str) -> GoogleUserClaims:
        try:
